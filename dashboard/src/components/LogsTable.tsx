        ...
        <div className="flex items-center border-b shrink-0 z-10" style={{ borderColor: 'var(--border-primary)', backgroundColor: 'var(--bg-secondary)' }}>
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 w-[130px] shrink-0">
            ...
            <div className="relative">
              <button
                className="flex items-center gap-1 px-2 py-1 text-sm text-text-primary hover:bg-gray-100 rounded transition-colors"
                onClick={() => setLimitOpen(!limitOpen)}
              >
                {limit}
                <LuChevronDown className="size-3" style={{ color: 'var(--text-secondary)' }} />
              </button>
              {limitOpen && (
                <div className="absolute top-full left-0 mt-1 py-1 min-w-20 rounded border bg-white shadow-md z-50" style={{ borderColor: 'var(--border-primary)' }}>
                  {limits.map((opt) => (
                    <button
                      key={opt}
                      className={`w-full text-left px-3 py-1 text-sm transition-colors ${opt === limit ? 'text-text-primary bg-gray-100 font-medium' : 'text-text-primary hover:bg-gray-50'}`}
                      onClick={() => { setLimit(opt); setLimitOpen(false) }}
                    >
                      {opt}
                    </button>
                  ))}
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 w-[150px] shrink-0">
            ...
            <div className="relative">
              ...
              {viewOpen && (
                <div className="absolute top-full left-0 mt-1 py-1 min-w-24 rounded border bg-white shadow-md z-50" style={{ borderColor: 'var(--border-primary)' }}>
                  {views.map((opt) => (
                    <button
                      key={opt}
                      className={`w-full text-left px-3 py-1 text-sm transition-colors ${opt === view ? 'text-text-primary bg-gray-100 font-medium' : 'text-text-primary hover:bg-gray-50'}`}
