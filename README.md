...
`curl -fsSL https://raw.githubusercontent.com/greplog/greplog/main/install.sh | sh && greplog dev` → dashboard in <60s. Zero Docker, zero config.
...
1. The installer script downloaded the precompiled `greplog` CLI binary for your platform and verified its sha256 against the release checksums.
